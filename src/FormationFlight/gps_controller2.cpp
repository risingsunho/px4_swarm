...
void ReceiveGPS(sensor_msgs::NavSatFix vel)
{
	...
	{
        ROS_INFO("!!!!WARNING!!!!");
        ROS_INFO("!!!!Flowers are in Collision Range!!!!");
        vec=CalDirection(U3GPS,CurrentGPS);
        vec.z=DesiredGPS.altitude;
        vec.x=5*vec.x;
        vec.y=5*vec.y;
	}
...
{
	U3GPS=vel;
	distance=CalDirection(CurrentGPS,U3GPS);
...
{
  ros::init(argc, argv, "gps_controller2");
  ros::NodeHandle n;
  ...
  ros::Subscriber mission_sub=n.subscribe("/target2", 1, ReceiveMission);
  ros::Publisher dir_pub = n.advertise<geometry_msgs::Vector3>("/direction2",10);
